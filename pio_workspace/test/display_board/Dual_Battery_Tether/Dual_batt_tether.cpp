...
Button buttons[] = {
  ...
  // Fifth Row
  //{0, 200, 78, 35, ILI9341_BLUE, "T"},
  //{80, 200, 78, 35, ILI9341_BLUE, "DB"},
  {160, 200, 78, 35, ILI9341_BLUE, "Box3"},
  {240, 200, 78, 35, ILI9341_BLUE, "Box4"},
};
...
void handleTouch() {
  ...
  // Check touch for tether box (left side, row 5)
  if (x >= 0 && x <= 78 && y >= 200 && y <= 235) {
    tether_dual_battery(tether_new, 0.0, 0.0);
    return;
  }
  // Check touch for dual battery box (next to tether)
  if (x >= 80 && x <= 158 && y >= 200 && y <= 235) {
    tether_dual_battery(0, 11.1, 11.7);  // example test values
    return;
  }
  /*
  ...
  */
 // Otherwise, check other buttons from buttons[] array
  for (int i = 0; i < sizeof(buttons) / sizeof(Button); i++) {
    Button btn = buttons[i];
    if (x > btn.x && x < btn.x + btn.width && y > btn.y && y < btn.y + btn.height) {
...
//this code doesnt have the real battery voltages, later when we combine